...
| Project company context | `$CWD/_conclave/state/memory/company.md` |
| Project preferences (overrides global) | `$CWD/_conclave/state/memory/preferences.md` |
...
2. Check if `$CWD/_conclave/state/memory/company.md` exists.
   - **Exists** → load it, load `$CWD/_conclave/state/memory/preferences.md`, load `$HOME/.conclave/core/security.policy.md`, then proceed to step 3.
...
4. **ELEUTHIA Profile Refresh check** — see [eleuthia.agent.md](../eleuthia.agent.md). Cheap (≤1s). If triggered, prepend a single-line refresh prompt above the menu.
5. **HEPHAESTUS Manufacturing Watch (auto-advisory)** — see [hephaestus.agent.md](../hephaestus.agent.md) Mode A. Cheap (≤2s). If forge candidates exist, prepend a single-line forge advisory above the menu.
6. **DOCS Freshness check** — Cheap (≤1s). Check if `$CWD/TUTORIAL.md` is missing or older than the newest `squad.yaml` or `skills/*/SKILL.md`:
   ```bash
   TUTORIAL="$CWD/TUTORIAL.md"
   NEWEST=$(find "$CWD/squads" "$CWD/skills" -name "squad.yaml" -o -name "SKILL.md" 2>/dev/null | xargs ls -t 2>/dev/null | head -1)
   if [ ! -f "$TUTORIAL" ] || { [ -n "$NEWEST" ] && [ "$NEWEST" -nt "$TUTORIAL" ]; }; then echo "stale"; fi
   ```
   If stale → prepend to menu output:
   ```text
   📘 DOCS: TUTORIAL.md desatualizado — /conclave docs pra regenerar
   ```
...
- **Initialize here** — scaffold a new `_conclave/` in `$CWD` (creates `_conclave/state/memory/`, `squads/`, copies template `company.md` and `preferences.md`).
...
1. `mkdir -p $CWD/_conclave/state/memory $CWD/squads $CWD/skills`
2. Copy template files from `$HOME/.conclave/core/templates/` (fallback: generate minimal stubs inline if the templates directory is missing):
   - `$HOME/.conclave/core/templates/company.md` → `$CWD/_conclave/state/memory/company.md` (contains `<!-- NOT CONFIGURED -->` marker, which triggers onboarding)
   - `$HOME/.conclave/core/templates/preferences.md` → `$CWD/_conclave/state/memory/preferences.md` (blank sections inherit from global)
...
6. Save the confirmed profile to `$CWD/_conclave/state/memory/company.md`.
...
| `/conclave tide` | 🌊 Run **POSEIDON Tide Observer** — aggregate streams, detect currents, propose actions ([poseidon.agent.md](../poseidon.agent.md)) |
...
DOCS
  /conclave docs             📘 Regenerate TUTORIAL.md from live squad/skill/agent data

GAIA SUBROUTINES (system observers — easter egg names from Horizon Zero Dawn)
...
4. Load company context from `$CWD/_conclave/state/memory/company.md`.
5. Load global preferences from `$HOME/.conclave/_memory/global-preferences.md` (if exists).
6. Load project preferences from `$CWD/_conclave/state/memory/preferences.md` (overrides global).
...
1. Read `$CWD/_conclave/state/memory/user-model.md` (fallback: `$HOME/.conclave/_memory/user-model.md`).
...
| Path | Owner |
| --- | --- |
| `$CWD/_conclave/state/memory/company.md` | project |
| `$CWD/_conclave/state/memory/preferences.md` | project |
...
**Location:** `$CWD/_conclave/runtime/logs/audit.jsonl` (project-local). If no project context exists (`$CWD/_conclave/` absent), fall back to `$HOME/.conclave/logs/audit.jsonl`.
...
echo '{"ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","event":"backup.created","flow":"edit-company","path":"_conclave/state/memory/company.md","backup":"_conclave/state/memory/company.md.bak-20260419-214530","reason":"pre-overwrite"}' \
...
`$CWD/_conclave/state/memory/session_logs.md` is a human-readable, curated narrative log of significant sessions — the "why and what changed" layer that `audit.jsonl` (machine events) and `runs.md` (per-squad runs) don't capture.
...
- **Auto-regenerate TUTORIAL.md (silent mode)** after any of these flows complete: squad created, squad edited, skill installed, skill uninstalled. Load [docs.agent.md](../docs.agent.md) in silent mode — no output to the user, no confirmation required. TUTORIAL.md is a system artifact, not a user-owned file; it has no overwrite protection and can always be rewritten freely.
...
- **Always append to the audit log** — every structural change (backup, squad create/edit/delete, company/preferences update, skill install/uninstall, run start/complete/fail) MUST append one line to `$CWD/_conclave/runtime/logs/audit.jsonl` per the [Audit Log](#audit-log) section.
