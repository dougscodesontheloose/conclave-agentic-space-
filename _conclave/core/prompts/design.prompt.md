...
- `_conclave/_memory/visual-identity.md` — Visual identity styles and content matching guidelines
- `_conclave/_memory/company.md` — Company context for personalization
- `_conclave/_memory/preferences.md` — User preferences (especially Output Language)
...
2. Check if `_conclave/_memory/research-cache/{cache-key}-{YYYYMM}.md` exists (where `YYYYMM` = current year-month).
...
4. **Cache miss or expired** (file absent or `created:` > 30 days):
   ...
   - After completing, write the cache file:
     ...
     ```
     Save to: `_conclave/_memory/research-cache/{cache-key}-{YYYYMM}.md`
