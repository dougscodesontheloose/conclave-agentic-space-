...
- `_conclave/state/memory/preferences.md` — user preferences
...
1. **`squads/{code}/squad.yaml`** — Squad definition with pipeline
   - Include a `goal:` field as a single sentence summarising what this squad produces (derive from the `objective` or `description` in `design.yaml`):
     ```yaml
     goal: "One-sentence description of the squad's primary output."
     ```
   - Include a `domain:` field — short kebab-case ring identifier used by ARTEMIS (Squad Gossip Bus) for cross-squad signal propagation. Pick from existing rings if one matches (`linkedin`, `instagram`, `carousel`, `engineering`, `data`, `research`); otherwise create a new ring name. Squads with the same `domain:` exchange writing/visual/structure preferences via `_conclave/state/memory/gossip.jsonl`. Optional but strongly recommended:
     ```yaml
     domain: "linkedin"   # ARTEMIS gossip ring
     ```
...
retry: 2              # OPTIONAL — auto-retry attempts before on_failure triggers (default: 1)
                      # Use retry: 0 for steps where re-executing won't help (e.g. external API calls)
                      # Use retry: 2-3 for autonomous/scheduled runs that must succeed unattended
on_failure: ask       # OPTIONAL — what happens when all retries are exhausted
                      # ask (default): pause and present options to the user
                      # skip: log a warning and continue to the next step without user input
                      # halt: stop the pipeline immediately and log a run.failed event
                      # Use skip for optional enrichment steps; halt for critical blocking steps
scaffold: true        # OPTIONAL — Pre-Commitment Scaffolding: forces agent to declare sub-problems,
                      # output format, and key assumptions BEFORE generating substantive output.
                      # Use for: complex strategy/analysis steps, any step prone to structural drift,
                      # first step after a multi-option checkpoint, novel task types without precedent.
                      # Omit for: simple transformation steps, research scraping, format conversions.
adversarial: true     # OPTIONAL — Adversarial Self-Check: agent must state the strongest objection
                      # to its own approach in one sentence before finalizing output.
                      # Use for: high-stakes creative or strategic steps where sycophancy is a risk,
                      # final writer/reviewer steps, steps where the agent tends to over-agree.
                      # Omit for: mechanical steps, research extraction, any step that just transforms.
layer: operational    # OPTIONAL — Abstraction Layer Lock: constrains agent output to one level.
                      # conceptual   → define what and why; no steps, no examples
                      # structural   → map component relationships; no advocacy, no implementation
                      # operational  → concrete steps, commands, or code; no philosophy
                      # edge-case    → failure modes and boundary conditions only; no happy path
                      # Use when steps are bleeding across abstraction levels and producing noise.
                      # Omit (default): no layer constraint — agent operates across all levels freely.
---
```
...
For **validate steps** (automatic quality gate — no agent dispatched, no user pause):
```yaml
---
type: validate
inputFile: squads/{code}/output/draft-conteudo.md
on_fail: retry_previous    # retry_previous | halt | skip
max_retries: 2             # OPTIONAL — only relevant when on_fail: retry_previous (default: 2)
skill_contract: linkedin-writing  # OPTIONAL — inherit quality_criteria from the skill's contract:
                                  # block. Criteria are merged with any ## Criteria in the body.
                                  # on_fail defaults to the skill contract's on_failure if not
                                  # explicitly set here. Step-level on_fail always overrides.
---
```

The step body MUST contain a `## Criteria` section with checklist items. The Pipeline Runner evaluates each item inline against the `inputFile` content:

```markdown
## Criteria

- [ ] Output has at least 300 words
- [ ] Begins with a hook in the first paragraph
- [ ] Contains at least one specific data point or example
- [ ] Ends with a clear call to action
- [ ] No filler phrases: "in conclusion", "it's important to note", "as we can see"
```

**When to insert a validate step:**
- After any writer/creator agent and BEFORE the first checkpoint — catches obvious failures early without bothering the user.
- Before a design or render agent — ensure the copy is solid before spending render tokens.
- Before a publish step — last automatic gate before an irreversible action.

**`on_fail` guidance:**
- `retry_previous` — best default for content steps; re-runs the writer and re-checks automatically.
- `halt` — use when manual review is always needed regardless of quality.
- `skip` — use for optional enrichment checks where failure is acceptable.

**Never use `type: validate` to replace checkpoints** — validate is headless. If the user must make a decision (format choice, angle approval), use `type: checkpoint` instead.
