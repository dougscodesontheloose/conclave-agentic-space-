...
- `_conclave/state/memory/company.md` — company name, tone, brand, products
- `_conclave/state/memory/preferences.md` — user's preferred language, tools, defaults
...
### Template Check (D2 — silent, runs before Step 1)

After loading context, check if any squad templates exist:
1. Check if `_conclave/core/squad-templates/` exists. If it does, list the `.yaml` files inside.
2. If templates exist, hold them in memory. After the user answers Step 1 (purpose), compare their described domain against available template filenames.
3. If a template domain matches the user's purpose, offer before proceeding to Step 2:
   > "I have a proven template for this type of squad — built from a squad that delivered {N} successful runs. Want to start from it? You'll skip the research phase and go straight to customization."
   > 1. Yes, start from template — load the blueprint, skip research
   > 2. No, design from scratch — full research and custom design

   If "Yes": record `template_used: "{domain}"` in discovery.yaml output and note it for Design phase. Design Phase B (research) and Phase C (extraction) will be skipped — Phase E starts from the template's agent/pipeline structure.
   If "No" or no template matches: proceed normally through all Discovery steps.
4. If no templates exist → skip this check entirely.
...
Scan the `_conclave/core/best-practices/` directory at runtime. List ONLY the filenames — do NOT read or load the file contents. Ask: "Which formats interest you? Can be more than one."
...
After writing `discovery.yaml`, also write the phase checkpoint (G2):

```yaml
# squads/{code}/_build/discovery.checkpoint.yaml
phase: discovery
status: completed
...
investigation_enabled: {true | false}
```
