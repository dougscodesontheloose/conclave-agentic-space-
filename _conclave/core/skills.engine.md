...
4. **Extract contract criteria** (if `contract:` block is present in frontmatter):
   Parse the `contract.quality_criteria` array. If present and non-empty, store in working memory as:
   ```
   skill_contracts["{skill-name}"] = ["criterion 1", "criterion 2", ...]
   ```
   Also store `contract.on_failure` (default: `halt`) as `skill_contract_on_failure["{skill-name}"]`.
   If no `contract:` block → skip silently (backward compatible).

5. **Append to the agent's context** after all agent instructions:
   ```
...
6. **Multi-skill injection**: When an agent declares multiple skills, inject them in the order
...
7. **Missing skill handling**: If a skill listed in an agent's frontmatter was not resolved
...
2. **Contract Validation** (advisory — only if `contract:` block is present):
   - `contract.quality_criteria` is a non-empty list of strings ✓
   - `contract.on_failure` is one of: `retry_previous`, `halt`, `skip` ✓
   - If `contract.inputs` is present: every entry has `name` and `description` ✓
   - If `contract.outputs` is present: every entry has `name` and `description` ✓
   Include contract status in the report:
   ```
   Contract:       ✅ Present — 5 quality criteria, on_failure: retry_previous
   ```
   or:
   ```
   Contract:       ⚪ Not defined (optional)
   ```

3. **Context Size Estimation**
...
**When to use:** Called automatically by the Pipeline Runner (D4) when a skill accumulates 3 consecutive `miss` or `partial` signals in `_conclave/state/memory/skill-signals.jsonl`. Can also be invoked manually via `/conclave skills` → Retrospective.

**Goal:** Identify the specific instruction failure and propose a targeted patch using a strict **Class-First Rubric**. This is a surgical edit — not a rewrite.

> **Restricted Sprawl Rule:** The retrospective process is strictly isolated. You must NEVER propose or apply changes to any file outside of the `skills/{skill}/` directory.

### Steps

1. **Load signal history**
   Read `_conclave/state/memory/skill-signals.jsonl` and filter entries for `{skill}`:
   ```bash
   grep '"skill":"{skill}"' "$CWD/_conclave/state/memory/skill-signals.jsonl" | tail -5
   ```
...
2. **Load the current skill definition**
   ...
   - The `contract.quality_criteria` array (if present) — these are the acceptance criteria against which failure will be evaluated
   - **Active-Update Bias (Sub-file Handling):** If the skill `type` is `script` or `hybrid`, check `script.path`. Read the referenced script file (e.g., `skills/{skill}/scripts/main.py`) alongside `SKILL.md`.

3. **Load recent run outputs (Clean Context)**
   For each squad in the signal history, find the most recent failed run output. Read ONLY the final output file and the specific failed `contract.quality_criteria`. Do NOT read the agent's intermediate reasoning trace or scratchpads, to ensure the retrospective fork sees a clean context.

4. **Diagnose the failure pattern**
   Cross-reference the signal history, the skill's instructions (or script), and the contract criteria evaluation:
   - If MCP-based: could be API limits, auth expiry, or changed endpoints
   - If prompt-based: could be instructions too vague, conflicting with agent persona, or missing edge case handling
   - If script-based: could be a logic error in the script itself, dependency drift, or missing env vars
   - If contract criteria exist: report which specific criteria are failing

5. **Propose a targeted patch (Class-First Format)**
   Draft a specific change using the rigid JSON Rubric below. Do not use conversational text or generic BEFORE/AFTER blocks.

   ```json
   {
     "target_file": "SKILL.md", // Or the specific script path, e.g., "scripts/main.py"
     "section": "## Process", // Markdown section or script function name
     "failure_mode": "instruction_gap", // Choose: instruction_gap, hallucination, dependency_error, logic_error, api_change
     "patch_content": "Exact replacement content to be applied"
   }
   ```
   ...
   Contract criteria failures (from last run):
     ✗ "Begins with a hook — not a question, not 'In today's world'"
   Diagnosis Rubric:
     File: {target_file}
     Section: {section}
     Mode: {failure_mode}
   ...
   Ask:
   > "Apply this structured patch to `{skill}`?"
   > 1. Yes — apply and bump version
   > 2. Edit manually — open the file
   > 3. Skip — mark as acknowledged

6. **If "Yes — apply and bump version":**
   - Apply the patch to `skills/{skill}/{target_file}`
   - If `SKILL.md` was not modified, still increment the `version` field in the `SKILL.md` frontmatter (patch bump: e.g. `1.0.0` → `1.0.1`)
   - Append one line to `_conclave/state/memory/skill-signals.jsonl` marking the retrospective:
...
7. **If "Skip — mark as acknowledged":**
...
1. **Load the candidate entry**
   Read the latest matching entry from `_conclave/state/memory/skill-candidates.jsonl`:

   ```bash
   grep '"squad":"{name}"' "$CWD/_conclave/state/memory/skill-candidates.jsonl" | tail -1
   ```
