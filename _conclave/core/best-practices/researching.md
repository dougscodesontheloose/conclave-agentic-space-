...
8. **Browser tool discipline** — Use native web search tools (WebSearch, web_fetch) for all public web research. Reserve browser automation (Playwright) for social media platforms, login-required pages, and visual/screenshot extraction. When using browser tools, **MUST** follow the Autonomous Loop (Observe -> Act -> Verify) defined in `browser-automation.md`. Never open a browser when a native search tool suffices.
