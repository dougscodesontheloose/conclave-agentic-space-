...
> **Companion document:** [`charter.md`](charter.md) — SafeGuard governs *data*; Charter governs *conduct*. The two are co-extensive and complementary. Neither overrides the other.
...
Agents MUST validate any dynamically generated or externally-sourced file paths before performing I/O operations. Use the `path_validator.py` script:
```bash
python3 _conclave/tools/scripts/path_validator.py <target_path>
```
If the script exits with a non-zero status (VETO), execution MUST stop immediately (Hard Veto).
