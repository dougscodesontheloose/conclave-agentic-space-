...
9. **Session Observer (COP)**: Initialize the breadcrumb trail for this session:
    ...
    python3 _conclave/tools/scripts/safe_write.py _conclave/runtime/scratch/session-breadcrumbs.jsonl '{"ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","session_id":"'"$(date +%Y-%m-%d)"'","type":"session_start","value":"Session initialized","squad":"none","quality":"pending"}'
    ```
...
- **STRICT WORKSPACE PINNING**: Subagents running scripts or terminal commands MUST explicitly set their current working directory (CWD) to their squad's designated workspace (e.g. `squads/{name}/workspace/`). Never rely on inherited directories from the orchestrator.
...
- **BREADCRUMB TRACKING (COP)**: During any session, after completing a significant action (file created/edited, analysis delivered, squad work done, architecture changed), drop a breadcrumb SILENTLY:
    ```bash
    python3 _conclave/tools/scripts/safe_write.py _conclave/runtime/scratch/session-breadcrumbs.jsonl '{"ts":"'"$(date -u +%Y-%m-%dT%H:%M:%SZ)"'","session_id":"'"$(date +%Y-%m-%d)"'","type":"{action|creation|squad_work|architecture|feedback}","value":"{1-sentence description, max 80 chars}","squad":"{squad-name or none}","quality":"{good|partial|miss|pending}"}'
    ```
    Rules:
