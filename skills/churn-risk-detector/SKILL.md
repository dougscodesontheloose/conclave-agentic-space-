...
|---|---|---|
| **Key findings** | `[OPERACIONAL]: churn-risk-detector — [finding]` | `[OPERACIONAL]: churn-risk-detector — Apify scraper returned 0 results for domain X, switched to direct mode` |
| **Parameter tuning** | `[OPERACIONAL]: churn-risk-detector — [param] works better as [value]` | `[OPERACIONAL]: churn-risk-detector — --max-results 50 is optimal for G2 (beyond 50 = mostly duplicates)` |
| **Strategic insights** | `[ESTRATÉGICO]: churn-risk-detector — [insight]` | `[ESTRATÉGICO]: churn-risk-detector — Competitor X has no case studies page, vulnerability for battlecard` |
