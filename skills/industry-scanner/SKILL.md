...
tags: [monitoring, research, strategy]
description_pt-BR: >
  Análise ampla do panorama setorial em 5+ dimensões: principais players, tendências, ameaças emergentes, oportunidades de mercado e sentimento do cliente. Produz um documento estratégico para agentes de pesquisa.
type: playbook
---

# Industry Scanner

**Core principle:** Tendências só importam quando mapeiam o problema em evolução do seu comprador.
...
1. **Setor/vertical** — Definição clara (ex: "AI dev tools" não "AI").
2. **Janela temporal** — 30, 90, 180 dias?
3. **Foco** — Tudo, ou recortes (funding, M&A, produto, contratações, content)?
4. **Output** — Report markdown? Tabela CSV? Slides?

> **Regra:** Estreite o escopo no Intake — "setor inteiro" sempre dá output raso.
...
| Apify (sinais sociais opcionais) | ~$1-3 por scan |
| LLM reasoning | Free |
| **Total típico** | **Free** ou **~$1-3** com Apify |
...
|---|---|---|
| **Key findings** | `[OPERACIONAL]: industry-scanner — [finding]` | `[OPERACIONAL]: industry-scanner — Apify scraper returned 0 results for domain X, switched to direct mode` |
| **Parameter tuning** | `[OPERACIONAL]: industry-scanner — [param] works better as [value]` | `[OPERACIONAL]: industry-scanner — --max-results 50 is optimal for G2 (beyond 50 = mostly duplicates)` |
| **Strategic insights** | `[ESTRATÉGICO]: industry-scanner — [insight]` | `[ESTRATÉGICO]: industry-scanner — Competitor X has no case studies page, vulnerability for battlecard` |
...
- "Quais tendências em [setor] eu deveria estar acompanhando?"
- "Faz um industry scan de [vertical]"
- "O que mudou em [setor] nos últimos 90 dias?"
- Mapeamento sistemático de movimentação setorial

**Auto-trigger:** Ative quando o usuário pede pulso de mercado, relatório de setor, mapa de tendências, ou análise de movimentação de indústria.
...
```env
APIFY_API_TOKEN=optional_for_social_signals
```

### Dependencies

Nenhuma obrigatória. `WebSearch` + `WebFetch` cobrem. Apify opcional para sinais de Reddit/X/LinkedIn.
