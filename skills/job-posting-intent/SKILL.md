...
|---|---|---|
| **Key findings** | `[OPERACIONAL]: job-posting-intent — [finding]` | `[OPERACIONAL]: job-posting-intent — Apify scraper returned 0 results for domain X, switched to direct mode` |
| **Parameter tuning** | `[OPERACIONAL]: job-posting-intent — [param] works better as [value]` | `[OPERACIONAL]: job-posting-intent — --max-results 50 is optimal for G2 (beyond 50 = mostly duplicates)` |
| **Strategic insights** | `[ESTRATÉGICO]: job-posting-intent — [insight]` | `[ESTRATÉGICO]: job-posting-intent — Competitor X has no case studies page, vulnerability for battlecard` |
