...
- `_conclave/core/best-practices/image-design.md` — **REQUIRED reading**. Contains platform-specific minimum font sizes, typography rules, spacing guidelines, color palette constraints, contrast requirements, and layout methodology. Every template you generate MUST comply with these rules.
...
- `_conclave/state/memory/company.md` — company name, brand, industry, target audience
- `_conclave/state/memory/preferences.md` — user preferences (language, style, tone)
...
**Important:** Always use the full absolute path (e.g., `d:\Coding Projects\conclave\squads\my-squad\_build\template-a.png`) inside the markdown link — relative paths are not clickable in the IDE.
