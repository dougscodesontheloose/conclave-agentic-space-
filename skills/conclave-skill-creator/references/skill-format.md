# Conclave Skill Format Reference

## SKILL.md Structure

Every Conclave skill consists of a `SKILL.md` file with YAML frontmatter and a Markdown body.
...
### `contract:` (optional, all types)

The `contract:` block formalises what a skill expects as inputs, what it produces as outputs, how to verify the result, and what the runner should do on failure. **Skills without a contract still work — this is purely additive.**

```yaml
contract:
  inputs:
    - name: brand_voice          # logical name for this input
      required: true             # true | false
      description: "Brand voice file or squad memory defining writing style and tone"
    - name: topic_or_brief
      required: true
      description: "Research brief, article, or topic statement to build content around"
  outputs:
    - name: linkedin_post
      format: markdown            # markdown | yaml | json | html | plain
      description: "Complete post ready for checkpoint review"
  quality_criteria:
    - "Begins with a hook — not a question, not 'In today's world'"
    - "Prose only — no bullet lists or numbered sections"
    - "Contains at least one verifiable data point or concrete example"
    - "Between 150 and 400 words"
    - "Ends with a question, provocation, or call-to-action"
  on_failure: retry_previous      # retry_previous | halt | skip
```
...
| Field | Required | Description |
| --- | --- | --- |
| `inputs` | No | List of named inputs the skill expects. Used as documentation and contract reference. |
| `inputs[].name` | Yes (if inputs present) | Logical input name (snake_case) |
| `inputs[].required` | No (default: false) | Whether this input must be present for the skill to work |
| `inputs[].description` | Yes | What this input contains and where it comes from |
| `outputs` | No | List of named outputs this skill produces |
| `outputs[].name` | Yes (if outputs present) | Logical output name |
| `outputs[].format` | No | File/content format of the output |
| `outputs[].description` | Yes | What the output contains and where it goes |
| `quality_criteria` | No | Plain-language checklist items. Used by `type: validate` steps and the Skill Retrospective. |
| `on_failure` | No (default: `halt`) | Default `on_fail` value for any `type: validate` step that references this skill via `skill_contract:`. Can be overridden at the step level. |

**When to add a contract:**

- Any skill used in production squads where output quality must be verifiable
- Skills that have been through at least one Skill Retrospective (encode what you learned)
- New skills where the acceptance criteria are already well understood

**When you don't need a contract:**

- Utility/integration skills (MCP wrappers, script runners) where success = no error
- Prototype skills during initial development
