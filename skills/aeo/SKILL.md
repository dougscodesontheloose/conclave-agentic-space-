...
Use `aeo` when the task requires its specific capabilities as described above.
...
|---|---|---|
| **Key findings** | `[OPERACIONAL]: aeo — [finding]` | `[OPERACIONAL]: aeo — Apify scraper returned 0 results for domain X, switched to direct mode` |
| **Parameter tuning** | `[OPERACIONAL]: aeo — [param] works better as [value]` | `[OPERACIONAL]: aeo — --max-results 50 is optimal for G2 (beyond 50 = mostly duplicates)` |
| **Strategic insights** | `[ESTRATÉGICO]: aeo — [insight]` | `[ESTRATÉGICO]: aeo — Competitor X has no case studies page, vulnerability for battlecard` |
