...
|---|---|---|
| **Key findings** | `[OPERACIONAL]: customer-win-back-sequencer — [finding]` | `[OPERACIONAL]: customer-win-back-sequencer — Apify scraper returned 0 results for domain X, switched to direct mode` |
| **Parameter tuning** | `[OPERACIONAL]: customer-win-back-sequencer — [param] works better as [value]` | `[OPERACIONAL]: customer-win-back-sequencer — --max-results 50 is optimal for G2 (beyond 50 = mostly duplicates)` |
| **Strategic insights** | `[ESTRATÉGICO]: customer-win-back-sequencer — [insight]` | `[ESTRATÉGICO]: customer-win-back-sequencer — Competitor X has no case studies page, vulnerability for battlecard` |
